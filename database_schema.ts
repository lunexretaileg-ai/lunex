...
export const categories = pgTable("categories", {
  ...
  description: text("description"),       // short category description
  coverImageUrl: text("cover_image_url"), // hero image for category page
});
...
export const productVariants = pgTable("product_variants", {
  ...
  color: text("color"),
  // deviceType per-variant allows one product to have e.g. both refurbished & used variants
  deviceType: text("device_type"), // 'new' | 'open_box' | 'refurbished' | 'assembled' | 'used' | null (inherits from product)
...
export const users = pgTable("users", {
  ...
  isAdmin: boolean("is_admin").default(false),              // kept for backward compat, derived from role
  createdAt: timestamp("created_at").defaultNow(),
});
...
export const orders = pgTable("orders", {
  ...
  paymentReceipt: text("payment_receipt"), // InstaPay sender handle or transfer reference
  merchantPaymentHandle: text("merchant_payment_handle"), // specific handle admin sets for this order
  cardLastFour: text("card_last_four"), // last 4 digits only — never store full card
  
  // InstaPay Payment Evidence fields submitted by the user
  transferAmount: text("transfer_amount"),
  transferSender: text("transfer_sender"),
  transferScreenshot: text("transfer_screenshot"),
...
export const chatSessions = pgTable("chat_sessions", {
  id: serial("id").primaryKey(),
  userId: integer("user_id").references(() => users.id),
  guestId: text("guest_id"), // For non-logged-in users (e.g., fingerprinted session ID)
...
export const chatMessages = pgTable("chat_messages", {
  ...
  senderId: integer("sender_id").references(() => users.id), // Null if sent by guest
  isAdmin: boolean("is_admin").default(false), // true if message sent by admin
