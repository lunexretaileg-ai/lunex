  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <section className="relative overflow-hidden pt-24 pb-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center max-w-3xl mx-auto">
            ...
            >
              {t('home.heroTitle', 'Full power.')}<br />
              <span className="text-gradient">{t('home.heroSubtitle', 'Refurbished right.')}</span>
            </motion.h1>
            ...
            >
              {t('home.heroDesc', 'Premium Apple devices professionally tested, guaranteed, and priced up to 40% less than retail in Egypt.')}
            </motion.p>
            ...
            >
              <Link href="/shop">
                <button className="px-8 py-4 rounded-full bg-foreground text-background font-semibold text-lg hover:scale-105 transition-transform w-full sm:w-auto shadow-xl shadow-foreground/10">
                  {t('home.shopAll', 'Shop All Devices')}
                </button>
              </Link>
              <Link href="/how-lunex-works">
                <button className="px-8 py-4 rounded-full bg-surface text-foreground font-semibold text-lg hover:bg-secondary hover:scale-105 transition-all w-full sm:w-auto">
                  {t('home.learnMore', 'How Lunex Works')}
                </button>
              </Link>
            </motion.div>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          <div className="grid md:grid-cols-3 gap-8 text-center">
            <div className="flex flex-col items-center">
              ...
              <h3 className="text-xl font-bold mb-2">{t('home.trust1Title', 'Professionally Tested')}</h3>
              <p className="text-muted-foreground text-sm">{t('home.trust1Desc', 'Every device undergoes a strict 40-point inspection before it reaches you.')}</p>
            </div>
            <div className="flex flex-col items-center">
              <div className="w-16 h-16 rounded-2xl bg-[hsl(var(--success))]/10 text-[hsl(var(--success))] flex items-center justify-center mb-6">
                <Coins className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-bold mb-2">{t('home.trust2Title', 'Unbeatable Value')}</h3>
              <p className="text-muted-foreground text-sm">{t('home.trust2Desc', 'Save up to 40% compared to brand new retail prices in Egypt.')}</p>
            </div>
            <div className="flex flex-col items-center">
              <div className="w-16 h-16 rounded-2xl bg-[hsl(var(--warning))]/10 text-[hsl(var(--warning))] flex items-center justify-center mb-6">
                <Zap className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-bold mb-2">{t('home.trust3Title', 'Warranty Included')}</h3>
              <p className="text-muted-foreground text-sm">{t('home.trust3Desc', 'Buy with confidence with our standard 3-6 month hardware warranty.')}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-end justify-between mb-12">
            <div>
              <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-4">{t('home.featuredTitle', 'Pristine Condition.')}</h2>
              <p className="text-muted-foreground text-lg">{t('home.featuredDesc', 'Our highest graded devices, ready to ship.')}</p>
            </div>
            <Link href="/shop?minCondition=9">
              <button className="hidden md:flex items-center text-primary font-semibold hover:opacity-80 transition-opacity">
                {t('home.viewAll', 'View all')} <ArrowRight className="w-4 h-4 ml-1 rtl:rotate-180" />
              </button>
            </Link>
          </div>
          ...
          <div className="mt-8 text-center md:hidden">
            <Link href="/shop">
              <button className="px-6 py-3 rounded-full bg-secondary text-secondary-foreground font-medium w-full flex items-center justify-center">
                {t('home.viewAll', 'View all')} <ArrowRight className="w-4 h-4 ltr:ml-2 rtl:mr-2 rtl:rotate-180" />
              </button>
            </Link>
          </div>
        </div>
      ...
      <section className="py-24 bg-surface rounded-t-[3rem]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="text-3xl md:text-4xl font-bold tracking-tight mb-12 text-center">{t('home.shopByCategory', 'Shop by Category')}</h2>
                ...
                <div className="group relative rounded-3xl overflow-hidden aspect-[4/5] cursor-pointer block">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-6">
                    ...
                    <div className="mt-2 flex items-center text-white/80 text-sm font-medium opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all">
                      {t('home.explore', 'Explore')} {t(`nav.${cat.name.toLowerCase()}`, cat.name)} <ArrowRight className="w-4 h-4 ml-1 rtl:rotate-180" />
                    </div>
                  </div>
                </div>
              </Link>
